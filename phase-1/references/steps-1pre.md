...
| None | **MANDATORY: Run Dashboard Structure Questionnaire (Setup-F below) BEFORE Stage A** |
...
## Step Setup-F: Dashboard Structure Questionnaire (MANDATORY if "None" in Setup-E)

**⚠️ CRITICAL: This step is MANDATORY when users select "None — starting fresh" in Setup-E.**

**Why it matters:** Without an existing dashboard or mockup, users often have vague ideas ("just a dashboard"). This questionnaire captures concrete structural decisions BEFORE Stage A requirements gathering, so requirements align with the intended layout.

**When to run:** Immediately after Setup-E if user selected "None"

**AskUserQuestion — Dashboard Structure Batch 1: Layout & Organization**

```
AskUserQuestion:
  header: "Dashboard structure"
  question: "How many dashboard sections/tabs do you envision?"
  multiSelect: false
  options:
    - label: "Single page / overview"
      description: "One screen, all metrics visible at once (3-8 widgets)"
    ...
    - label: "Drill-down structure"
      description: "Summary → Details → Granular (multi-level navigation)"
    - label: "Not sure yet"
      description: "Will decide based on metrics"
```

**AskUserQuestion — Dashboard Structure Batch 2: Visualization Types**

```
AskUserQuestion:
  header: "Visualization preferences"
  question: "What visualization types do you need? (Select all that apply)"
  multiSelect: true
  options:
    - label: "KPI cards (summary numbers)"
      description: "Large numbers showing key metrics (Revenue: $4.5M, Customers: 15K)"
    - label: "Trend charts (line charts over time)"
      description: "Revenue trend last 30 days, Customer growth over months"
    - label: "Comparison charts (bar charts)"
      description: "Revenue by region, Sales by product category, Performance by team"
    - label: "Distribution charts (pie/donut charts)"
      description: "Market share by segment, Customer breakdown by tier"
    - label: "Detailed tables (rows & columns)"
      description: "Transaction-level data, Order details with sorting/search"
    - label: "Heatmaps or matrices"
...
**AskUserQuestion — Dashboard Structure Batch 3: Filtering & Interactivity**

```
AskUserQuestion:
  header: "Filter & interactivity needs"
  question: "How should users interact with the dashboard?"
  multiSelect: false
  options:
    - label: "Static dashboard (no filters)"
      description: "Display fixed metrics, no user interaction"
    - label: "Global date filter only"
      description: "Adjust date range, all metrics update together"
    - label: "Global filters (date + dimension)"
      description: "Filter by date, region, segment, etc. — affects all metrics"
    - label: "Per-section filters"
      description: "Global filters + tab-specific filters (e.g., global date + regional filter on tab 2)"
    - label: "Advanced (all filters + cross-filter interactions)"
      description: "Filters affect each other (select region → update product options)"
```

**AskUserQuestion — Dashboard Structure Batch 4: Drill-Down & Detail Depth**

```
AskUserQuestion:
  header: "Drill-down & detail requirements"
  question: "Do users need to drill into detailed data?"
  multiSelect: false
  options:
    - label: "Summary only (aggregated view)"
      description: "No drill-down, no individual records"
    - label: "One level deep (summary → breakdown)"
      description: "Click on bar chart → see breakdown by category (e.g., Revenue → Revenue by product)"
    - label: "Multiple drill-down levels"
      description: "Revenue → Region → Product → Customer → Individual transactions"
    - label: "Detailed transaction table with search"
      description: "Full data table with sorting, filtering, search capability"
    - label: "Not needed — summary is enough"
      description: "Only aggregated KPIs and trends"
```

**AskUserQuestion — Dashboard Structure Batch 5: Performance & Update Frequency**

```
AskUserQuestion:
  header: "Performance & data freshness"
  question: "What are your performance and freshness requirements?"
  multiSelect: false
  options:
    - label: "Daily snapshot (built once/day, static file)"
      description: "Dashboard updated nightly, shared as static HTML file"
    - label: "Multiple times daily"
      description: "Rebuilt 2-4 times per day (morning, midday, evening, EOD)"
    - label: "Hourly or near real-time"
      description: "Refreshed every hour or within 15 minutes of data arrival"
    - label: "Live/on-demand queries"
      description: "Query data fresh each time dashboard loads (slow but latest)"
    - label: "Not sure — recommend based on data volume"
      description: "Will decide based on data size and complexity"
```

**Output to state.md:**
```markdown
## Phase 1 — Dashboard Structure (Setup-F)

**Structure Preferences:**
- Layout: [Single page / 2-3 tabs / 4+ tabs / Drill-down]
- Visualizations: [types selected]
- Filtering: [static / global / per-section / advanced]
- Drill-down: [summary / one level / multiple / detailed table / none]
- Performance Target: [daily / multiple daily / hourly / live / tbd]
```

**ENFORCEMENT:**
- ⚠️ **All 5 batches are MANDATORY** — do not skip or combine
- ⚠️ **Record answers in state.md** — these shape Stage A requirements (metrics, dimensions, layout decisions)
- ⚠️ **If user says "not sure"** → Record as `[TBD]` and revisit during Stage A metric definition (Step 1b)

**Next Step:** After Setup-F completes, proceed to Stage A (Steps 1a–1o in `./steps-1a-1o.md`)
