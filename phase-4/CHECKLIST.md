...
## Track A: Extract Reusable Skill (Steps 4a-0 to 4a-vi)
...
- [ ] **4b-iv:** Configure knowledge bases — copy `agent-prompt-template.md` → `system_prompt.md`, fill `dashboard_tables.yml` (use `{name, td_query}` format, not a plain string list); **check `system_prompt.md` ≤ 9,000 chars with `wc -c`** before pushing
